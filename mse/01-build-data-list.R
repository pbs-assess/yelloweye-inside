...
SRA_data$length_bin <- 0.1 * (SRA_data$Len_bins[-length(SRA_data$Len_bins)] + 25)
SRA_data$maxage <- 80

# Data list
MO <- readRDS("data-generated/yelloweye-rockfish-ins-privacy.rds")
...
generate_indices <- function() {
  ...
  #HBLL
  HBLL <- readRDS("data-generated/hbll-joint-index.rds")
  ...
  # Dogfish
  dogfish <- readRDS("data-generated/dogfish-index.rds")
  ...
  Ind_2011 <- readxl::read_excel("data-generated/commercial_cpue.xlsx")
...
generate_CAL <- function() {
  ...
  res <- readxl::read_excel("data-raw/SC Rec YellowEye Biodata.xlsx", sheet = 2) %>% filter(!is.na(`LENGTH(MM)`))
  res$Len_bin <- cut(res$`LENGTH(MM)`, breaks = SRA_data$Len_bins, labels = SRA_data$Len_bins[-length(SRA_data$Len_bins)])
  rec_len_matrix <- summarise(group_by(res, YEAR, Len_bin), n = n()) %>% reshape2::acast(list("YEAR", "Len_bin"), value.var = "n", fill = 0)
...
generate_CAA <- function() {
  ...
  comm_age <- comm_age %>% full_join(data.frame(age2 = 1:SRA_data$maxage)) %>% reshape2::acast(list("year", "age2"), value.var = "n", fill = 0)
...
generate_survey_CAA <- function() {
  ...
  s_age <- s_age %>% full_join(data.frame(age2 = 1:SRA_data$maxage)) %>% reshape2::acast(list("year", "age2"), value.var = "n", fill = 0)
...
sel_com <- 1/(1 + exp(-log(19) * (c(1:SRA_data$maxage) - 14.4) / (21 - 14.4)))
SRA_data$vul_par <- cbind(sel_com, sel_rec)
