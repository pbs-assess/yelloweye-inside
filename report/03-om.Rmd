...
For Inside Yelloweye Rockfish, we established four reference-set OMs encompassing variation in: (1) the weight given to the dogfish survey; (2) the magnitude of the historical catch during the period 1986-2005; (3) uncertainty in future recruitment; and (4) selectivity in the HBLL survey (Table \@ref(tab:ye-scen)).  
We further established two robustness-set OMs encompassing possible additional sources of uncertainty: (1) a scenario that assumes lower natural mortality than the other OMs; and (2) a scenario that assumes a higher CV in the future HBLL survey (Table \@ref(tab:ye-scen)).
...
*TODO: Issue #3. Wondering if we should call OM (1) something like "Base" rather than "Upweight dogfish survey"? Upweight dogfish survey awkwardly implies that it is a modification of another OM ... which it was originally but we trashed that OM. Then we can spend more time describing its settings and sources. It will be much easier to cite when describing the other OMs.*

#### (1) Upweight dogfish survey {#sec:approach3-reference1}
...
Projected recruitment deviations were sampled in log space with standard deviation $\tau = 0.4$ and autocorrelation $\theta_\textrm{AC} = 0.8$, with $\theta_\textrm{AC}$ estimated post-hoc from the SRA model (see Appendix A of @anderson2020gfmp).
...
Projecting only one index of abundance is consistent with many data-limited MPs, which only utilize a single index of abundance (see Appendix \@ref(app:mps)).
The projected index values were generated with standard deviation $\sigma_I = 0.25$ in log space.

This "Upweight dogfish survey" OM formed the baseline OM, upon which all the other OM scenarios were based.
...
A catch reconstruction was done by @haigh2011, which is used in the other scenarios. This reconstruction included doubling the nominal catches of Yelloweye Rockfish for the period 1986 until 2005, as industry did not have confidence in the catch data for those years [@dfo2012].
...
In this scenario, an extreme recruitment event is generated on average once every generation (38 years), with extreme recruitment years randomly drawn from *TODO an XX distribution (mean, SD)?.*
If a year is randomly selected as an extreme recruitment year, then the recruitment is sampled from a lognormal distribution with standard deviation $\tau = 2$.

<!-- TODO: Quang: add an equation to describe episodic recruitment -->

#### (4) Estimate HBLL selectivity {#sec:approach3-reference4}

There is a lack of commercial age-composition data for this stock. Furthermore, annual sample sizes of age-composition data from the HBLL survey are very small (Appendix \@ref(app:biological-data)). *TODO: Add Quang's age-comp fits to Conditioning section below*
...
from a lognormal distribution with $M \sim Lognormal(0.045, 0.2)$.
...
The "Low M" scenario used $M \sim Lognormal(0.025, 0.2)$, reflecting the possibility that the stock could be less productive than assumed in the reference set scenarios.

#### (B) Higher CV HBLL {#sec:approach3-referenceB}

The "Higher CV HBLL" scenario considers the possibility that the projected simulated HBLL index could be too precise in the simulations.
In this scenarion we used the standard deviation and autocorrelation derived from the HBLL index residuals in the base OM.
*TODO: Need to explain what this means in plain language*
...
The SRA model conditions estimates of the following OM parameters: 

* unfished recruitment
* initial depletion (in year $t_c$)
* relative fishing mortality
* annual recruitment deviations and autocorrelation in recruitment
* selectivity parameters (if estimated)

The SRA was run with 250 replicates.
Each replicate fits a different value of M and h (sampled independently from the distributions shown in Appendix \@ref(app:desc-om-yelloweye)).
Selectivity for commercial fisheries was fixed to the same values used in the recent Outside Yelloweye Rockfish rebuilding plan for the commercial hook and line fishery [@cox2020].
*add rec fisheries*
Selectivity for the HBLL survey was estimated using available age-composition data ... *ONLY IN THE Estimate HBLL selectivity scenario - check what was done in the fixed scenario* 
Selectivity in the Dogfish Survey was set to mirror selectivity in the HBLL survey, whether it was fixed or estimated. 
Multinomial annual sample sizes for age-composition data were based on the number of sampling trips in each year. 

*TODO: define a trip. What does "based on" mean? sample sizes was the number of trips or some function of number of trips?*

Initial attempts to fit the SRA model did not produce good fits to the observed indices of abundance.
Furthermore, retrospective analysis showed persistent retrospective bias in annual estimates of spawning biomass when sequentially removing 11 years of data (when evaluated at the mean values of *M* and the steepness of the stock-recruit relationship *h*, Mohn's $\rho = 0.25$; @mohn1999).
For long-lived species like Yelloweye Rockfish, values of Mohn's $\rho > 0.2$ should raise concerns about model specification [@hurtado2015].
...
Upweighting the likelihood weight for the SRA fit to the Dogfish Survey ($\lambda^I_s = 4$; Equation B.22 in @anderson2020gfmp) removed this retrospective bias in estimates of spawning biomass (Mohn's $\rho = -0.05$, evaluated at the mean of the distributions for *M* and *h*). 
However, when this was done, the estimated age-at-full-selectivity in the HBLL survey was estimated to be 40+ years.
This is inconsistent with the age-composition data, where the modes of the HBLL age-composition data occur at around 20 years.
...
*TODO: fact check the last sentence. What is the official name of that survey?*

Therefore, in the present analysis, for the "Base" scenario, and all other scenarios except "Estimate HBLL selectivity", it was decided to fix the age-at-full selectivity of the HBLL Survey to 20 years, while also upweighting the dogfish survey. This fit generated small retrospective bias in spawning biomass (Mohn's $\rho = -0.07$, evaluated at the mean of the distributions for *M* and *h*).

Retrospective bias among these fits was sensitive to the value of *M*, with larger retrospective bias in spawning biomass estimates at higher *M* (Figure \ref(fig:mohns-rho).

(ref:fig-mohns-rho) Values of Mohn's $\rho$ for three different model configurations: "Initial fit", "(1) Base", "(4) Estimate HBLL selectivity", across a range of values for *M*.

```{r mse-chart-basic, fig.cap="(ref:fig-mse-chart-basic)", out.width="6.3in"}
knitr::include_graphics(here::here("mse/figures/conditioning/retrospective_M.png"))
```
