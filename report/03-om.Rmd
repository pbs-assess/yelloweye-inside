...
Parameters under the four components are entered into "slots" [terminology referring to a feature of the "S4" object-oriented programming system in R; @r2019], described in detail in Appendix B of @carruthers2018 and Appendix A of @anderson2020gfmp.
...
To isolate the effects of specific sources of uncertainty on performance of MPs, we recommend development of alternative OMs that change the value (or distribution) of one or more parameters and/or data sources of interest (see next section).
...
Statistical diagnostics or retrospective analyses can then be used to generate a suitable set of OMs.
This is also known as conditioning the OMs.

DLMtool's companion software package, MSEtool [@huynh_msetool_2019], includes an extremely efficient implementation of a stock reduction analysis (SRA) [@kimura1982; @walters2006], which is effectively a catch-at-age model that estimates the combinations of historical fishing mortality and recruitment that would be consistent with the observed data.
...
The historical period is calibrated to historical observations using an age-structured SRA, fully described in Appendix B of @anderson2020gfmp.
...
Where possible, parameter settings for Inside Yelloweye Rockfish OMs were derived from biological data from the inside Hard Bottom Longline (HBLL) survey (Appendix \@ref(app:biological-data)).
Other parameters were derived from the scientific literature and stock assessments for Yelloweye Rockfish in other areas.
...
The OMs were conditioned using age-composition data from the inside Hard Bottom Longline (HBLL) survey (Appendix \@ref(app:biological-data)), survey indices (Appendix \@ref(app:index-data)) and commercial and recreational catch data (Appendix \@ref(app:catch-data)).
...
These authors recommend  that the reference set of OMs include the most important uncertainties, which should be both highly plausible and have major impacts on results. 
@punt2016 provide a list of factors which commonly have a large impact on MSE performance due to uncertainty (their Table 3).
...
These may include effects related to environmental change, structural representation of population dynamics, or alternative data sources.
...
More details on visualization of performance metrics from the robustness and reference sets of OMs are provided in Section \@ref(sec:results).

For Inside Yelloweye Rockfish, we established four reference-set OMs encompassing variation in: (1) the weight given to the dogfish survey; (2) the magnitude of the historical catch during the period 1986-2005; (3) uncertainty in future recruitment; and (4) selectivity in the HBLL survey.  

We further established two robustness-set OMs encompassing possible additional sources of uncertainty: (1) a scenario that assumes lower natural mortality than the other OMs; and (2) a scenario that assumes a higher CV in the future HBLL survey.
...
The OM scenarios are listed in Table \@ref(tab:ye-scen) and described below.
...
Each replicate fits a different value of M and h (sampled independently from the distributions shown in Appendix \@ref(fig:desc-stock-maxage-yelloweye)).
