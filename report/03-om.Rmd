...
This results in calibrated estimates of model parameters and estimates of historical biomass and fishing mortality (in years $t_1$ to $t_c$), which are consistent with historical observations.
...
Where possible, we derived OM settings for Inside Yelloweye from biological data from the inside Hard Bottom Longline (HBLL) survey (Appendix \@ref(app:biological-data)).
...
The conditioning SRA model exhibited better retrospective behaviour when the SRA model was fit with more weight applied to the Dogfish Survey in the likelihood function, with selectivity in the HBLL survey fixed at 20 years (see Section \@ref(sec:approach3-conditioning) below).
...
For consistency with @yamanaka2011, all scenarios except (2) included doubling the nominal catches of Yelloweye Rockfish for the period 1986--2005, as industry did not have confidence in the catch data for those years, indicating that total catch may have been double the nominal catch for that period [@dfo2012; see  Appendix \@ref(app:catch-data), Section \@ref(sec:com-catch-data)].
For this scenario, to test model sensitivity to the assumption of large unreported catch, we did not double the nominal catch for the period 1985--2005.
...
For example, very large or even extreme year classes have been estimated for several BC rockfish species [*TODO: DANA examples: POP?, Boccacio, Yelloweye??*].
...
The parameter $\eta_y$ is a Bernoulli random variable $\eta_y \sim \textrm{Bernoulli}(p = 1/38)$, which indicates whether an extreme recruitment event will occur. 
...
Given high uncertainty in our choice of selectivity, we allowed the SRA to estimate selectivity for the HBLL survey in this scenario, utilizing the available age-composition data. <!-- this description is a bit confusing as it indicates the HBLL selectivity is fixed and estimated. need to specify which OMs had fixed selectivity in the sentence above --> 

### ROBUSTNESS SET {#sec:approach3-robustness}

#### (A) Low M {#sec:approach3-referenceA}

The base model assumed a mean value for natural mortality *M* of 0.045 $y^{-1}$, drawn 
from a lognormal distribution with $M \sim \textrm{Lognormal}(0.045, 0.2)$.
This was based on the value used in the last stock assessment for the Inside Yelloweye Rockfish stock [@yamanaka2011]. 
