...
Where possible, we derived OM settings from all available survey biological data from area 4B, primarily from the inside HBLL surveys (Appendix \@ref(app:biological-data)).
...
These are: (1) aggregated reporting of rockfish as Other Rockfish (ORF; rockfish species other thatn Pacific Ocean Perch) and (2) the magnitude of unreported catch that was discarded at sea prior to the introduction of 100% at-sea monitoring in the groundfish longline fleet in 2006 [@stanley2009].
...
This makes estimation of selectivity in the surveys highly uncertain, which led to the choice to fix survey selectivity.

Given large uncertainties in our choices of selectivities, we allowed the SRA to estimate selectivity for the surveys in this scenario, utilizing the available survey age-composition data.
...
In this case, combined fishery selectivity for the projection period is replaced with the SRA-calibrated estimates of fishing mortality-at-age in the final year ($t_c$) of the historical period, normalized by dividing by apical fishing mortality in that year.
...
However, note that the DLMtool MPs only utilize a single index of abundance in the projection period (see Appendix \@ref(app:mps)).
In the current study, all MPs (except for the constant-catch MPs) utilize the HBLL survey.
...
Each replicate used a different value of *M* and *h* (sampled independently from the distributions shown in Appendix \@ref(app:desc-om-yelloweye), except for Scenario (A), which used a different distribution for *M*).
The model was initialized under the assumption that spawning biomass ($B_y$) was in an unfished equilibrium state prior to 1918, the first year of the time series, i.e., $B_1 = B_0$.
...
Increasing the likelihood weight for the SRA fit to the Dogfish Survey ($\lambda^I_s = 4$; Equation B.22 in @anderson2020gfmp) and fixing age-at-full-selectivity in the HBLL survey to 20 years removed the retrospective pattern (Figure \@ref(fig:retro-initial), bottom panel). 
...
(ref:fig-retro-initial) Retrospective patterns in spawning biomass for the initial fit and Scenario (1). Coloured lines represent estimates of spawning biomass with $X$ years of data removed, where $X$ is shown in the legend for each series.
...
There are a number of large uncertainties in selectivity in the Dogfish survey due the absence of biological data from this survey, and changes in fishing operations and hook type that occurred in 2004 (Appendix \@ref(app:index-data), Section \@ref(sec:dogfish-index-data)).
Selectivity in the Dogfish survey was set to mirror selectivity in the HBLL survey, whether it was estimated (Scenario (4)) or fixed (all other scenarios).

Due to a lack of biological data from commercial and recreational fisheries, selectivity in these gears was fixed in all OM scenarios (Figure \@ref(fig:fishery-selectivity)).
...
All replicates achieved convergence in all scenarios.
For all scenarios, the estimated index fell within the observed confidence intervals in most years, although there were some outliers (Figure \@ref(fig:survey-fits)).
...
The SRA model fit the survey age-composition data reasonably well, despite very small sample sizes (Figures \@ref(fig:sra-conditioned-comp-fit1) to \@ref(fig:sra-conditioned-comp-fitB)).
...
In several years, the SRA predicted larger abundance in the plus group (80+ *y*) than in the observed data. 
...
In addition, large structural uncertainties in the model and available data (e.g., the Dogfish survey, catch data) may have resulted in misspecification of relative age-compositions.
...
SRA model fits to the survey age composition data for Scenario (1), showing observed (points) and estimated (green lines) proportions. Sample sizes (N) are based on the number of sampling trips in each year.
...
Estimated mean and standard deviation for reference points *F*~MSY~, *B*~MSY~ and MSY are provided in Table \@ref(tab:sra-ref-pts).
Note the large standard deviations due to the large range of sampled values for *M* and *h* (Appendix \@ref(app:desc-om-yelloweye), Sections \@ref(app:desc-stock-m-yelloweye) and \@ref(app:desc-stock-h-yelloweye)).
...
Therefore, according to all the reference set scenarios and one robustness set scenario, the stock can already be considered to have rebuilt above the LRP.
...
Note that trajectories are identical for Scenarios (1), (3) and (B), as Scenarios (3) and (B) only differ from Scenario (1) in the treatment of parameters in the projection period.
...
In all scenarios except Scenario (A), the current median spawning biomass is estimated to be above the LRP.
The median spawning biomass is estimated to be above the USR in Scenarios (1) and (2); and estimated to be below the USR in scenarios (4) and (A).
...
Estimates of historical recruitment deviations were similar across scenarios (Figure \@ref(fig:recdev-om)).
...
The assessment results triggered a rebuilding plan [@ifmp2018], which is being updated here.
...
The SP model set $B_{MSY}$ to 50% of unfished biomass, set the initial biomass in 1918 to 90% of unfished biomass, and used a prior probability distribution for the intrinsic rate of population growth (*r*) as a likelihood penalty.
...
Model results more closely matched those of the previous assessment, estimating the stock to be below the LRP (Figure \@ref(fig:spm-biomass)).
The SP model estimated the reference points *B*~MSY~ = 1844 t, *F*~MSY~ = 0.03 *y*^-1^, and MSY = 62 t.
MSY and *F*~MSY~ estimates were similar to those from the SRA Base OM (Table \@ref(tab:sra-ref-pts)), but the estimate of *B*~MSY~ was approximately 35% higher for the SP model than the SRA. 

An alternative Fox model (not shown), where $B_{MSY}$ was approximately 37% of unfished biomass, did not appreciably change the perception of stock status.

Results suggest that differences in estimated stock status between the current OMs and the previous stock assessment [@yamanaka2011] are in large part due to the choice of model structure.
We note that very similar findings were made by @cox2020 in the rebuilding plan for the outside stock of Yelloweye Rockfish.
Their analyses, also based on age-structured models, also predicted the current stock status to be above the LRP, i.e., not in need of rebuilding. 
