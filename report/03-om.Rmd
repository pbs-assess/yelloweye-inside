...
The historical period is calibrated to historical observations using the SRA [see Appendix B of @anderson2020gfmp].
...
In the "Low catch" scenario, we test model sensitivity to the assumption of large unreported catch for the period 1986--2005.
...
This scenario addresses the concern that very large cohorts are not adequately modelled by the lognormal distribution for recruitment deviations that was used in Scenario (1).
In the Episodic recruitment scenario, recruitment deviations $\varepsilon_{R,y}$ for each year are generated as:
...
This makes estimation of selectivity in the surveys highly uncertain, which led to the choice to fix survey selectivity in the other scenarios.
...
Lower values of natural mortality have been used for Inside Yelloweye Rockfish [@yamanaka2001; @cosewic2008; @wood2019]. This scenario used a lower mean in the distribution for *M*, with $M \sim \textrm{Lognormal}(0.025, 0.2)$, reflecting the possibility that the stock could be less productive than assumed in the other scenarios. 
...
In this case, fishery selectivity in the OM for the projection period is replaced with the SRA-calibrated estimates of fishing mortality-at-age in the final year  of the historical period ($t_c$), normalized by dividing by apical fishing mortality in that year.
...
However, the estimated plus group was only slightly reduced in most years for this scenario (Figure \@ref(fig:sra-conditioned-comp-fit2)).
...
Scenario (A) estimated median spawning biomass to have been below the LRP for most years after 2000 and less than 50% probability of being above the LRP in 2019 (Figure \@ref(fig:ref-pt)).
...
However, to explore the effects of model structure on stock status, we fit a SP model to the currently-available catch, CPUE and fishery-independent indices of abundance. 
...
An alternative Fox model (not shown) did not appreciably change the perception of stock status.
