...
Therefore, in the present analysis, for Scenario (1), and all other scenarios except Scenario (4), it was decided to fix the age-at-full selectivity of the HBLL Survey to 20 years (Figure \@ref(fig:HBLL-selectivity), top left panel), while also upweighting the dogfish survey.

*TODO: Why was 20 years chosen? To match the commercial LL selectivity for the outside stock? PHMA selectivity for Outside YE was futher to the right with full selectivity around 30, looks like, from the plots*  

*TODO: what is the parameter for age-at-full-selectivity? I think this gets set using Length at full selectivity? Did you change the LFS parameter and eyeball the selectivity curves until they approximated the desired shape?*

Selectivity in the Dogfish Survey was set to mirror selectivity in the HBLL survey, whether it was fixed or estimated.
There are a number of uncertainties in selectivity due to changes in fishing operations and hook type in 2004 (Appendix \@ref(app:index-data), Section \@ref(sec:dogfish-index-data)). 
*TODO: Check this is true for Scenario (4)?*

Due to a lack of biological data from commercial and recreational fisheries, selectivity in these gears was also fixed across OM scenarios (Figure \@ref(fig:fishery-selectivity)). Parameter values were chosen so that the selectivity-at-age curves approximated those estimated for the commercial longline and recreational fisheries in @cox2020 for the Outside stock of Yelloweye Rockfish.

(ref:fig-retro-initial) Retrospective patterns in spawning biomass for the initial fit and Scenario (1) *TODO: write properly*.

```{r retro-initial, fig.cap="(ref:fig-retro-initial)", out.width="5.3in"}
...
(ref:fig-HBLL-selectivity2) Selectivity-at-age for the HBLL survey, which was fixed for all OM scenarios except Scenario (4). Note that selectivity in the Dogfish Survey was set to mirror the selectivity in the HBLL survey.

```{r fishery-selectivity, fig.cap="(ref:fig-fishery-selectivity)", out.width="6.3in"}
...
ref_pts %>%
  ...
  rename("FMSY (/y)" = FMSY, "BMSY (t)" = BMSY, "MSY (t)" = MSY) %>% 
  csasdown::csas_table(caption = "Estimated reference points for each OM Scenario") %>% 
...
```{r spm-biomass, fig.cap="(ref:fig-spm-biomass)", out.width="\\textwidth"}
