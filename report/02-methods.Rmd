...
## THE MANAGEMENT PROCEDURE FRAMEWORK {#sec:methods-mp-framework}

We follow the Management Procedure Framework (MP Framework) for selecting MPs for setting catch limits for data-limited groundfish stocks, developed by @anderson2020gfmp.
The MP Framework utilizes the software packages DLMtool [@carruthers2018], MSEtool [@huynh_msetool_2019] and gfdlm [@gfdlm], implemented in the statistical software R [@r2019].
This rebuilding plan represents the first application of the MP Framework for providing science advice to support catch decisions.
...
The framework also identifies a critical first step, defining the decision context [@gregory2012 and  @cox2016].
...
Following selection and implementation of the MP for setting the catch limit, it is also necessary to periodically monitor and evaluate the performance of the MP [@dowling2015a; @carruthers2018]. 
This may be done through informal means, such as through feedback from fishers and survey information [e.g., @cox2008a], or through more formal statistical measures, where observed data are compared to predictions from the OMs to test whether the system is performing as expected [@carruthers_hordyk_2018; discussed in @anderson2020gfmp].

Next, we describe the approach used for developing a candidate rebuilding plan for Inside Yelloweye Rockfish, following the six best practice steps.

## BEST PRACTICES FOR IMPLEMENTING A MANAGEMENT PROCEDURE APPROACH {#sec:methods-approach}

### STEP 1: DEFINE THE DECISION CONTEXT {#sec:methods-approach1}
...
### STEP 2: SELECTION OF OBJECTIVES AND PERFORMANCE METRICS  {#sec:methods-approach2}
...
Current rebuilding guidance only provides for objectives to build stocks out of the critical zone [@ifmp2018], with milestones intended to ensure progress is achieved during the rebuilding process.
...
However, this is supposed to be achieved beyond the period of the rebuilding plan.
We have interpreted this milestone in its strictest sense (i.e., the rebuilding plan is to rebuild the stock out of the critical zone), but the milestone may be revised in the future to state that positive biomass trends should be achieved until the stock is in the healthy zone.
...
*TODO: define the years for calculating LRP 1.5 GT, LRP 1 GT, USR 1.5 GT and LT C20*
  ...
  * LRP 1.5 GT		P[B > 0.4 BMSY] after 1.5 GT (56 years)
...
@anderson2020gfmp suggested calculating performance statistics across replicates and the time window as defined for the performance metric.
For example, we recommend calculating the *F*~MSY~ performance metric across all replicates and years simultaneously, whereas short-term (ST) catch metrics are averaged over only the years 2020-2029.

#### COSEWIC CONSIDERATIONS {#sec:methods-approach2-cosewic}
...
### STEP 4: IDENTIFICATION OF CANDIDATE MANAGEMENT PROCEDURES  {#sec:methods-approach4}
...
### STEP 5: SIMULATION OF THE APPLICATION OF THE MANAGEMENT PROCEDURES  {#sec:methods-approach5}
...
### STEP 6: PRESENTATION OF RESULTS AND SELECTION OF MANAGEMENT PROCEDURE  {#sec:methods-approach6}
