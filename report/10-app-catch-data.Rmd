...
The commercial catch time series used in this analysis (Figure \@ref(commcatch2) and Table \@ref(tab:commcatch)) differs from the one previously reported in 2009, for several reasons. Ongoing quality control and updates to the groundfish catch database may result in minor differences in the data over time (Maria Surry, DFO, Pacific Biological Station, pers. comm., March 9, 2020). As well, an earlier version of the catch reconstruction algorithm was used to develop the time series for the previous stock assessment, as the final version had not yet been published. Further refinements to the reconstruction algorithm resulted in significant changes to the estimated historical catch in some years (Norm Olsen, DFO, Pacific Biological Station, pers. comm., March 9,2020).

The reconstruction algorithm could have been applied to all years in the timeseries until to 2005 (after which, full at-sea and dockside monitoring came into effect). However, for this analysis we used the reconstructed catch data from 1918-1985, and switched to the nominal catch data in 1986. The nominal catch from 1986-2005 was then doubled to calibrate our OMs,to be consistent with the previous assessment [@yamanaka2011]. Prior to the previous assessment, DFO saught advice from industry on the reconstructed catch [@DFO2012]. In 2009, advice from industry representatives indicated that the scale of unreported catch was probably equal to landed catch, and they did not have confidence in the catch reconstruction between 1986 and 2005. As a result of this advice, the catch for these years was doubled. Because discards are estimated as part of the catch reconstruction algorithm, we did not use the reconstructed catch for the period 1986-2004, to avoid double counting of discards. As a sensitivity check on this assumption, in the current analysis we explore one OM scenario where the 1986-2005 commercial catches were not doubled (the "low catch" scenario, section \@ref(sec:approach3-reference2)).
...
rec_catch <- rec_catch %>% mutate_if(is.numeric , replace_na, replace = "--") 
...
fsc_catch <- fsc_catch %>% mutate_if(is.numeric , replace_na, replace = "--")
