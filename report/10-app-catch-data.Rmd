...
The reconstruction algorithm could have been applied to all years in the time series until to 2005 (after which, full at-sea and dockside monitoring came into effect). However, for this analysis we used the reconstructed catch data from 1918--1985, and switched to the nominal catch data in 1986. The nominal catch from 1986--2005 was then doubled to calibrate our OMs,to be consistent with the previous assessment [@yamanaka2011]. Prior to the previous assessment, DFO sought advice from industry on the reconstructed catch [@dfo2012b]. In 2009, advice from industry representatives indicated that the scale of unreported catch was probably equal to landed catch, and they did not have confidence in the catch reconstruction between 1986 and 2005. As a result of this advice, the catch for these years was doubled. Because discards are estimated as part of the catch reconstruction algorithm, we did not use the reconstructed catch for the period 1986--2005, to avoid double counting of discards. As a sensitivity check on this assumption, in the current analysis we explore one OM scenario where the 1986--2005 commercial catches were not doubled (the "low-catch" scenario, section \@ref(sec:approach3-reference2)). 
...
We followed the same method as in Yamanaka et al. (2011) to estimate the recreational catch of Inside Yelloweye Rockfish for 1982 to 1999, the average proportion of Yelloweye to total Rockfish catch was calculated for each PFMA in 2000 and 2001. The average proportions were then used to derive estimates of Yelloweye catch from the total Rockfish catch by PFMA between 1982--1999. To estimate catch of Yelloweye in PFMA 12 for the years 1982--1999, the average proportion of Yelloweye in total Rockfish catch over 2000 and 2001 was calculated and applied to the total Rockfish catch for the rest of the SOG in each year (Table \@ref(tab:recbyarea)). To be consistent with the previous assessment, an adjustment of 1.09 was applied to total annual effort to account for lack of records in PFMA 12, where effort was not recorded prior to 2000. We converted rockfish pieces to weight by multiplying by 2.49 kg, which was the average weight of Yelloweye sampled in the creel surveys between 2000 and 2008.

\clearpage

```{r reccatch, echo=FALSE, warning=FALSE, results="asis"}
rec_catch <- read.csv(here("data-raw/rec-allcatch-effort.csv"), stringsAsFactors=FALSE, header=TRUE)
...
csasdown::csas_table(rec_catch,
  ...
  digits = 1,
  repeat_header = TRUE,
  caption = "Reconstructed (1918--1981) and modern (1982--2019) recreational catch estimates for Inside Yelloweye Rockfish (tonnes)."
)
```
...
Yelloweye Rockfish are an important traditional food source for coastal First Nations in BC [@eckert2018], including in the inside waters of 4B. Total FSC catch of Yelloweye Rockfish is not available for either the historic or contemporary time period, and the available data is not resolved to the species level (M. Fetterly, DFO Policy Treaty Support, pers. comm., November 7, 2019 and  A. Rushton, DFO South Coast Fisheries Management, pers. comm., February 7, 2020).
...
The only FSC data that are available are from the commercial dockside monitoring program (DMP) between 2006 and 2019 (Table \@ref(tab:fsc-catch)). These data were collected from "dual fishing" trips, which occur when indigenous fishers choose to keep some of the catch obtained during a commercial fishing trip for FSC purposes.
Both commercial and FSC catch are monitored during the offload.
Between 0 and 0.8 T amounting to an average of 5.6% of the total commercial catch has been landed on dual fishing trips in this time period.
...
Most of the dual fishing trip records are from the northern part of the study area because this is also where most of the commercial fishing for Yelloweye Rockfish in 4B occurs.
...
Similar to how some of the FSC catch is accounted for within the commercial catch data, some FSC effort from small boats will be captured in the recreational data from the creel survey program.
