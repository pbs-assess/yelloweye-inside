...
The reconstruction algorithm could have been applied to all years in the timeseries until to 2005 (after which, full at-sea and dockside monitoring came into effect). However, for this analysis we used the reconstructed catch data from 1918-1985, and switched to the nominal catch data in 1986. The nominal catch from 1986-2005 was then doubled to calibrate our OMs,to be consistent with the previous assessment [@dfo2012]. Prior to the previous assessment, DFO saught advice from industry on the reconstructed catch [@dfo2012]. In 2009, advice from industry representatives indicated that the scale of unreported catch was probably equal to landed catch, and they did not have confidence in the catch reconstruction between 1986 and 2005. As a result of this advice, the catch for these years was doubled. Because discards are estimated as part of the catch reconstruction algorithm, we did not use the reconstructed catch for the period 1986-2004, to avoid double counting of discards. As a sensitivity check on this assumption, in the current analysis we explore one OM scenario where the 1986-2005 commercial catches were not doubled (the "low catch" scenario, section \@ref(sec:approach3-reference2)). *TODO: We used doubled commercial catches from 1986 to 2005 so these should be reflected in this table. Only the Low catch scenario used nominal catch from 1986-2005 *
...
```{r recplot, echo=FALSE, warning=FALSE, results="asis"}
...
rec_area <- read.csv(here("data-raw/rec-catcheffortbyarea.csv"), stringsAsFactors=FALSE, header=TRUE)
...
Indigenous fishers from the member First Nations represented by the A-Tlegay Fisheries Society, are largely obtaining Yelloweye Rockfish on dual fishing trips (Christa Rusel, pers. comm.* DANA: Date?*).

In the southern part of the study area, indigenous fishers have little commercial capacity
FSC catch in the Strait of Georgia is, therefore, primarily from small recreational boats (C. Ayers, pers. comm.*DANA: date?*).
...
The proportion of FSC fisher encountered by the dockside creel monitor was not, however, readily available in the recreational database (CREST) (P. Zetterberg, pers. comm. *DANA: date?*).
...
```{r mgt-changes, echo=FALSE, warning=FALSE, results="asis"}
