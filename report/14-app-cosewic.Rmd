...
2% decline rate on- we estimated that Strait of Georgia lingcod were depressed to 2% B0, and have been recovering from that, so thought that we could use this level. Robyn has a ref to justify the 5%B0 level for the Criteria E.(Forrest et al.2015 defined a stock as 'extirpated' if the biomass was 5% of B0 or less). We applied that threshold here and although 5% arbitrary, simulation testing is needed to find a more appropriate threshold on a stock-specific basis -->
...
COSEWIC metric A results for each operating model for Inside Yelloweye Rockfish. 
...
*The only cosewic metric used was metric E, extinction risk (where extinction = 2%B0 or 5%B0). I can't find any reported results for cosewic metric A (P of 70% decline over last 3 generations...etc.). There are results for metric E for each MP and OM (ref:fig-cosewic-all) and then the average of all OMs for each of 2% and 5%.  Is that your understanding as well?*
...
(ref:fig-cosewic-all)
COSEWIC risk of extinction metrics for Inside Yelloweye Rockfish by OM. 
...
(ref:fig-cosewic-avg)
COSEWIC risk of extinction metrics averaged across OMs for Inside Yelloweye Rockfish. 
