...
We conditioned the operating models using indices of abundance from the Hard Bottom Long Line (HBLL) Inside survey and the Strait of Georgia Dogfish longline survey. Survey design and modelling of indices for each survey are described here.   
...
$$
A_{i,t} = - \frac{ \left(\log(P_{i,t} \right)}{1 - P_{i,t}}.
...
The value 0.002438 corresponds to 8 ft between hooks (per km), 0.009144 to an assumed 30 ft area swept around the set that fish are catchable (per km), and 1000 scales the area swept from km to m.
...
We use a spatiotemporal model to estimate the density of Yelloweye Rockfish per km^2^. We calculated the area swept by multiplying the number of hooks deployed by the estimated width swept by the length of two gagnions and the estimated distance between hooks (8 ft spacing).
