...
However, several irregularities have occurred:
...
```{r hbll-spde, out.width="4in", eval=FALSE, fig.cap="Stochastic Partial Differential Equation (SPDE) mesh from INLA. 400 knots. kmeans clustering algorithm. Samples from all years are shown simultaneously."}
...
```{r hbll-residuals, eval=FALSE, fig.cap="Randomized quantile residuals from the model shown in space and time.", out.width="\\textwidth"}
include_figs("hbll-joint-residual-map.png")
```
...
include_figs("dogfish-joint-spde.png")
```

```{r, fig.cap="Spatial residuals with some jittering.", out.width="\\textwidth"}
include_figs("dogfish-residual-map.png")
```
