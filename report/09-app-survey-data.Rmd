...
For these reasons, we chose to undertake a geostatistical spatiotemporal model standardization of the HBLL index [e.g., @shelton2014; @thorson2015; @anderson2019synopsis] to account for the irregular implementation of the survey design (Section \@ref(sec:hbll-spatiotemporal)).
...
The HBLL survey catch is mostly comprised of North Pacific Spiny Dogfish (*Squalus suckleyi*; hereafter "dogfish"), which are considered a major hook competitor with rockfishes [@obradovich2018].
...
See @anderson2019synopsis (their Appendix G, Section G.5) for further on the details on the hook competition correction.
...
The study only compared catch rates for dogfish; however, the hook change likely affected catchability for Yelloweye Rockfish differently than for dogfish.
...
Nonetheless, it is important to note that this correction factor is based on a limited number of sets and Yelloweye Rockfish and is driven largely by five sets with one or two more YE caught by circle hooks than J hooks and two sets with 12 and four YE caught by circle hooks vs. zero by J hooks (Table \@ref(tab:dogfish-hook-comparison)).
...
The standardized index for YE for an average hook type indicates a decline in relative abundance from the 1980s to the next survey in 2004, but with considerable uncertainty (Figure \@ref(fig:dog-standardized-index)).
