...
The SRA was able to fit to the HBLL survey relative biomass index and XXX indices reasonably well for all OM scenarios (Figure \@ref(fig:survey-fits)).
The XXX survey was better fit than the XXX, likely due to XXX (Figure \@ref(fig:survey-fits)).
...
The majority of replicates were retained (Table \@ref(tab:sra-fraction)). However, by discarding  replicates from non-converged models, the input parameters carried through to the conditioned OMs varied slightly (Figure \@ref(fig:sra-conditioned-parameters2)).
The discarded parameter values represent implausible parameter combinations according to the SRA model.
The implied spawning stock biomass depletion trajectories during the historical period from the eight OMs are similar with the exception of the "Low M" scenario (Figure \@ref(fig:depletion-om)).
...
We started with the full set of provisional candidate MPs described in Appendix \@ref(app:MPs) (Table \@ref(tab:mps)).
...
We could apply these criteria to either the average (Figures \@ref(fig:tigure-avg)) or minimum performance (Figures \@ref(fig:tigure-min)) and such a decision is likely best made by a technical working group or decision-makers coupled with stakeholders in an applied setting.
...
First, we chose to retain only one satisficed SP model MP (.SP8040_0.6) since the SP model MPs that differed only in production-function shape (.SP6040_0.6_fox) or HCR (.SP6040_0.6 and .SP4010_0.6) had near identical performance (Figure \@ref(fig:tigure-min)).
Second, we removed CC_hist20 since it was outperformed by the other constant catch MPs---LT LRP and STC were both exceeded by all other constant-catch MPs (Figure \@ref(fig:tigure-min)).
...
There was variability in performance among the individual reference-set scenarios with the most variability in both the long-term and entire projection period performance metrics: LT USR, FMSY, LTC, and AADC (Figure \@ref(fig:tigure-panel)).
...
Long-term catch (LTC) was roughly comparable between a fishing strategy of fishing at *F*/*F*~MSY~ and the satisficed Itarget or .SP8040_0.6 MPs (Figure \@ref(fig:dot-lines)).
Short-term catch (STC) of all satisficed MPs exceeded these reference MPs (Figure \@ref(fig:dot-lines)).
Fixed catch MPs of 1.1 or 1.2 times average catch from the last five years rendered similar performance to other satisficed MPs across most performance metrics within the reference-set scenarios, with somewhat higher LTC and AADC metrics (Figure \@ref(fig:dot-lines)).
These fixed-catch MPs do not incorporate feedback between the management and biological systems.

Trade-offs among performance metrics for most of the satisficed MPs were relatively minor within the reference-set scenarios (Figures \@ref(fig:tradeoff-reference) and \@ref(fig:spider-satisficed-mps-avg)).
Within the "Ceq. 200%" scenario, there was a slight trade-off between STC and LT LRP (Figure \@ref(fig:tradeoff-reference)).
...
```{r proj-index, fig.cap="Historical and projected SYN WCVI relative biomass index values. Vertical dashed line represents 2019. The shaded region represents the 95\\% quantile and individual lines represent four sample replicates.", out.width="\\textwidth"}
...
```{r proj-scenarios, fig.cap="(ref:fig-proj-scenarios)", out.width="\\textwidth"}
