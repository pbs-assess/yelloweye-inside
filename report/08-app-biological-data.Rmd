...
```{r age-freq, fig.cap="(ref:fig-age-freq)", out.height="8in"}
library(here)
...
$$
\log (W_i) \sim \operatorname{Student\mhyphen t}
(df = 3, \log(a) + b \log(L_i), \sigma)
$$
...
=======
\begin{align}
L_i &\sim \mathrm{Log-normal}\left( \log(l_\mathrm{inf} (1 - \exp(-k (A_i - t_0)))), \sigma \right),
\end{align}

where $L_i$ and $A_i$ represent the length and age of fish $i$, $l_\mathrm{inf}$, $k$, and $t_0$ represent the von Bertalanffy growth parameters, and $\sigma$ represents the log standard deviation, or scale parameter. The term $- \sigma^2 /2$ represents a 
...
```{r prop-mature, fig.cap="(ref:fig-prop-mature)", out.width="\\textwidth"}
library(here)
...
```{r mat-months, fig.cap="(ref:fig-mat-months)", out.width="\\textwidth"}
library(here)
