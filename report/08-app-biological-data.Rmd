...
(ref:fig-age-freq) Age-frequency plot for inside Yelloweye Rockfish from the hard-bottom 
...
Size and age structure of Yelloweye and other rockfish have been shown to be truncated under high fishing pressure for the central BC coast which also influences fecundity as larger females show an increasing number of eggs per unit of body weight [@mcgreer2017].
...
longline surveys (northern and southern) in inside (area 4B) waters (HBLL INS N/S). Female fish are shown as coloured bars and male fish are shown behind as light grey bars. The total number of fish measured for a given survey and year are indicated in the top left corner of each panel.
...
```{r length-age, fig.cap=paste0("Length-age model fits and plots for inside Yelloweye Rockfish. Text on the panels shows the parameter estimates and open grey circles represent individual fish that the models are fit to. These figures include all survey samples."), out.width="0.6\\textwidth"}
library(here)
...
## Summary table of biological data availability
Biological research survey data are maintained in GFBio, the Groundfish Biological Samples database [@databases2019]. The above plots include all available research data for Yelloweye Rockfish from all surveys in area 4B (Figure \@ref(fig:map-4B)), except for the age and length frequency plots (Figures \@ref(fig:age-freq), \@ref(fig:length-freq)) which include only HBLL INS N/S survey data. A summary of all available survey data for Yelloweye Rockfish in area 4B is presented in Table \@ref(tab:survey-data).

```{r survey-data, results='asis'}
