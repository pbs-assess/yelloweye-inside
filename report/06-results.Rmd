...
Multidimensional trade-offs were challenging to interpret via a radar plot for this closed-loop-simulation (Figure \@ref(fig:spider-satisficed-mps-avg)), which exemplifies some of the warnings about radar plots offered in @anderson2020gfmp.
We included radar plots here to gauge reader interest at the Regional Peer Review meeting and to illustrate their pitfalls for the MP Framework.
...
We note that none of the biomass projections predicted the stock to be below the LRP in the first year of the projection period, and all MPs achieved continuous growth throughout the projection period, albeit at different rates.
The reference MP FMSYref indicates that catches could be increased without increasing the likelihood of falling below LRP. This is due to current high probability that the stock is currently above LRP across the reference set scenarios. 
<!--TODO: check this is this true. We should show those results from the SRA (see note at the end of 03-om.Rmd-->
...
Within the reference set and across all satisficed MPs, only the low catch scenario predicted some replicates that had *F* > *F*~MSY~ and *B* < *B*~MSY~ (and *B* < 0.4 *B*~MSY~) by the final projection year (Figure \@ref(fig:kobe)).
...
In contrast to the reference set, where there was little trade-off between the ST C10 and LRP 1.5GT performance metrics, Scenario (A) demonstrated a short-term catch vs. long-term conservation tradeoff (Figure \@ref(fig:tradeoff-robust)).
Scenario (B) demonstrated little trade-off between ST C10 and LRP 1.5GT.
Similar to the reference set, we found the radar plots challenging to interpret (Figure \@ref(fig:spider-satisficed-mps-robust)).
...
There was increased TAC variability across replicates and within individual replicates in Scenario (B) compared to the reference set OM projections (e.g., Figure \@ref(fig:proj-high-index-cv) vs. \@ref(fig:proj-updog-fixsel)).
...
This occurred across the MPs, although the least so for the CC_10t MP, and, among the Islope MPs, it occurred the least for Islop_10_lambda04 (Figure \@ref(fig:kobe)).
...
Note that sensitivity of the projections does not necessarily correspond to sensitivity of the rank order of MPs---which is likely more important in a decision context (Figures \@ref(fig:tigure-panel) and \@ref(fig:tigure-panel-rob)).
