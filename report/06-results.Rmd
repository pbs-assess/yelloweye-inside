...
Within the reference set, there was little to no trade-off between LTP 1.5GT and ST C10 since all were able to achieve LRP 1.5 GT > 0.99 (Figures \@ref(fig:tradeoff-reference)).
Multidimensional trade-offs are challenging to interpret via a radar plot for this closed-loop-simulation (Figure \@ref(fig:spider-satisficed-mps-avg)) and exemplify some of the warnings about rader plots offered in @anderson2020gfmp.
...
(ref:fig-proj-upweight-dogfish) Same as Figure \@ref(fig:proj-updog-fixsel) but for the OM "(4) Higher steepness".
...
The "Low M OM more easily shows the conservation vs. yield tradeoff. To meet satisficing criterion for LRP 1.5GT, ST C10 must be very low.
