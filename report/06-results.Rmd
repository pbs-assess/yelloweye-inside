...
Within the reference set and across all MPs, only the Scenario (2) predicted some replicates that had *F* > *F*~MSY~ and *B* < *B*~MSY~ (and *B* < LRP) by the final projection year (Figure \@ref(fig:kobe)).
...
The FMSYref and FMRYSref75 MPs illustrate that catches would have to be lower in Scenario (A) than in the reference-set OMs to achieve similar conservation outcomes (e.g., Figure \@ref(fig:proj-low-m) vs. \@ref(fig:proj-updog-fixsel)).
