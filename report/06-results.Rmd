...
Within the reference set, there was virtually no trade-off between LTP 1.5GT and ST C10 since all MPs were able to achieve LRP 1.5 GT > 0.99 (Figures \@ref(fig:tradeoff-reference)).
...
The "(A) Low M" robustness scenario lowered the probability of achieving the LRP 1.5GT performance metric from > 0.99 probability in the reference set to a range of 0.75--0.90 (Figure \@ref(fig:tigure-panel-rob) vs. \@ref(fig:tigure-panel-ref)).
We saw similar declines in the other conservation performance metrics.
The "(B) High HBLL CV" robustness scenario slightly lowered the probability of achieving the ST C10 performance metric (Figure \@ref(fig:tigure-panel-rob) vs. \@ref(fig:tigure-panel-ref)) presumably because of the added error in the projected HBLL index.
...
In contrast to the reference set, where we saw little trade-off between the ST C10 and LRP 1.5GT performance metrics, the "(A) Low M" robustness OM demonstrates a short-term-catch vs. long-term conservation tradeoff (Figure \@ref(fig:tradeoff-robust)).
The "(B) High HBLL CV" robustness scenario again demonstrates little trade-off between ST C10 and LRP 1.5GT.
Similarly to the reference set, we find the radar plots challenging to interpret but include them to gauge reader interest at the Regional Peer Review meeting and as an illustration of their pitfalls for the MP Framework (Figure \@ref(fig:spider-satisficed-mps-robust)).

The projection plots illustrate the increased TAC variability across replicates and within individual replicates in the "(B) High HBLL CV" compared to the reference set OM projections (e.g., Figure \@ref(fig:proj-high-index-cv) vs. \@ref(fig:proj-updog-fixsel)).

In the projection plots for "(A) Low M", the CC_15t MP can take the stock to very low biomass levels in some replicates (Figure \@ref(fig:proj-low-m)).
