...
Performance within the reference set of OM scenarios varied across satisficed MPs (hereafter just "MPs") and performance metrics (Figures \@ref(fig:tigure-panel) and \@ref(fig:dot-lines)).
...
In contrast to the reference set, where there was little trade-off between the ST C10 and LRP 1.5GT performance metrics, Scenario (A) demonstrated a short-term catch vs. long-term conservation tradeoff among MPs (Figure \@ref(fig:tradeoff-robust)).
Scenario (B) demonstrated no trade-off between ST C10 and LRP 1.5GT since all MPs achieved LRP 1.5GT with probability > 0.99.
...
For Scenario (A), the Kobe plots indicated that a number of replicates resulted in *F* > *F*~MSY~ and *B* < 0.4 *B*~MSY~  (Figure \@ref(fig:kobe)). 
...
By comparing the projections across scenarios on the same figure, it is clear that *F*/*F*~MSY~, *B*/*B*~MSY~, and catch were most sensitive to the Scenario (A) assumptions (Figure \@ref(fig:proj-scenarios)). 
