...
Objectives may span a wide range of policy or legislated objectives (e.g., maintaining the stock above the LRP), economic objectives (e.g., maintaining an average catch or variability in catch), and cultural objectives (e.g., maintaining minimum required access to the stock or specific fishing areas).
...
4. Given the above conservation objectives are achieved, maintain a target catch in the short and long term.
...
We propose the following performance metrics to measure the objectives, where *B* represents spawning biomass, MSY refers to maximum sustainable yield, *B*~MSY~ refers to equilibrium spawning biomass at MSY, GT represents generation time, and AADC represents average absolute deviation in catch ($C$) over $n$ years (note: *ST* = short term, *LT* = long term). We define the LRP and USR as 0.4*B*~MSY~ and 0.8*B*~MSY~, respectively, following the provisional definitions in the PA Framework [@dfo2006], as used in the 2010 stock assessment [@yamanaka2011]. In the closed-loop simulations, all reference points and performance metrics are calculated in the OM.

1. **LRP 1.5GT**: P(*B* > 0.4 *B*~MSY~) after 1.5 GT (56 years)
2. **USR 1.5GT**: P(*B* > 0.8 *B*~MSY~) after 56 years
3. **LRP 1GT**: P(*B* > 0.4 *B*~MSY~) after 1 GT (38 years)
4. **ST C10**: P(catch > 10 t) during 2020--2029  <!-- RF: shouldn't this be 2021-2030??-->
5. **ST C15**: P(catch > 15 t) during 2020--2029
<!-- 6. ST C20: P(catch > 20 t) during 2020--2029 RF: we don't report this one-->
6. **LT C20**: P(catch > 20 t) after 1 GT (38 years)
...
$$
\textrm{AADC}_\textrm{2020-2029} = \dfrac{1}{8}\sum_{y=2021}^{2029} \mid C_y - C_{y-1} \mid.
$$
...
$$
\textrm{AADC}_\textrm{2012-2019} = \dfrac{1}{6}\sum_{y=2013}^{2019} \mid C_y - C_{y-1} \mid.
$$
