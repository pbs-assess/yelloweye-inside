...
- GB_slope_lambda1:   $\lambda = 1$	  $I^\textrm{slope}_{y-1}$ is calculated from index in the preceding 5 years

- GB_slope_lambda05:  $\lambda = 0.5$	$I^\textrm{slope}_{y-1}$ is calculated from the preceding 5 years 

- GB_slope_yrs10:     $\lambda = 1$	  $I^\textrm{slope}_{y-1}$ is calculated from the preceding 10 years
...
where $\delta_\textrm{min}$ is the most negative drop allowed in the relative biomass index before the fishery is closed that year and $\Delta I_y$ is the current index value divided by the last observed index value.
...
Note that the slow-up--fast-down behaviour due to how $\delta_\textrm{min}$ and $\delta_\textrm{max}$ are set here, combined with the simulated survey index variability, can result in the MP "ratcheting" down the TAC despite an overall increasing index (e.g., Figure \@ref(fig:proj-not-satisficed-eg)).
...
This does not stop the TAC from slowly being lowered below $\textrm{TAC}_\textrm{Floor}$ by the "ramp" described by $(1 + \Delta I_y ) \textrm{TAC}_{y-1}$ if $\delta_\textrm{min} \lt \Delta I_y \leq \delta_\textrm{max}$.
