...
To determine which MPs would be carried forward as satisficed MPs [@miller2010; discussion in @anderson2020gfmp], we began by assessing average and minimum performance across all candidate MPs for the reference set of OMs (Figures \@ref(fig:tigure-avg) and \@ref(fig:tigure-min)).
All MPs met the satisficing criterion (LRP 1.5GT > 0.95), both in individual OM scenarios in the reference set, and when averaged across all four reference set scenarios (Figures \@ref(fig:tigure-avg) and \@ref(fig:tigure-min)).
...
This resulted in retention of Fixed TACs of 10 t and 15 t and Islope MPs (Figure \@ref(fig:tigure-avg)).
...
For the two performance metrics used for satisficing (LRP 1.5GT > 0.95 and ST C10 > 0.50), average LRP 1.5GT was `r round2(max(pm_avg_sat$"LRP 1.5GT"))` for all MPs and ST C10 varied between `r round2(min(pm_avg_sat$"ST C10"))` and `r round2(max(pm_avg_sat$"ST C10"))` (Figure \@ref(fig:tigure-avg) and \@ref(fig:tigure-panel)).
...
Using these criteria, MPs are "satisficed" if cells in *both* "`r names(satisficed_criteria)[1]`" and "`r names(satisficed_criteria)[2]`" are outlined.
...
In other words, this figure illustrates the worst-case performance of each MP across the reference set of OMs.
...
<!-- TODO: RF - curious that IDX reduces catches when biomass is increasing-->
