...
We ran the closed-loop simulations across 250 stochastic replicates using DLMtool version 5.4.2, `r rversion`, and the simulation random seed set to 1. The length of the projection period was 100 years to facilitate calculation of extinction risk for COSEWIC assessment (Appendix \@ref(app:cosewic)).
...
We set the following criteria to determine which MPs are satisficed: LRP 1.5GT > 0.9 and ST C10 > 0.5.
...
All MPs met the satisficing criterion (LRP 1.5GT > 0.9), both in individual reference set scenarios and averaged across all four reference OMs (Figures \@ref(fig:tigure-avg) and \@ref(fig:tigure-min)).
...
This resulted in constant-catch TACs of 10 t and 15 t and Islope MPs being satisficed (Figure \@ref(fig:tigure-avg)).
...
(ref:fig-tigure-avg) Average performance of all candidate MPs across the reference set scenarios.
...
(ref:fig-tigure-min) Minimum performance of all candidate MPs across the reference set scenarios.
...
In other words, this figure illustrates the worst performance of each MP across the reference set scenarios.
...
Examples of MPs that were not satisficed for Scenario (1). 
