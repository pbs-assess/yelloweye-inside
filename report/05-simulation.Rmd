...
This left us with Fixed TACs of 10 and 15 t and Islope MPs (Figures \@ref(fig:tigure-avg)).
...
Between the two performance metrics used in the initial satisficing step, average LRP 1.5GT was `r round2(max(pm_avg_sat$"LRP 1.5GT"))` and ST C10 varied between `r round2(min(pm_avg_sat$"ST C10"))` and `r round2(max(pm_avg_sat$"ST C10"))` across MPs within the reference set (Figure \@ref(fig:tigure-avg) and \@ref(fig:tigure-panel)).
Minimum LRP 1.5GT was `r round2(max(pm_min_sat$"LRP 1.5GT"))` and ST C10 varied between `r round2(min(pm_min_sat$"ST C10"))` and `r round2(max(pm_min_sat$"ST C10"))` across MPs within the reference set (Figure \@ref(fig:tigure-min) and \@ref(fig:tigure-panel)).
