...
The Islope MPs showed some contrast across MPs within the reference set for ST C10 and ST C15, depending on the MP configuration and the scenario.
...
Candidate MPs should perform well across both reference and robustness scenarios [@rademeyer2007].
...
This low mortality/low productivity scenario was the only one where the median biomass was estimated to be in the critical zone at the start of the projection period.
...
Steller Sea Lion abundance was greatly reduced due to hunting and predator controls, but have now recovered to historic levels [@olesiuk2018; and see Appendix F]. 
...
Our Scenario (4), where we estimated survey selectivity-at-age, resulted in lower estimates of stock status, implying that assumptions about selectivity were a contributing factor to perceptions of stock status (see Section \@ref(sec:discussion-uncertainties-selectivity)).
...
These may occur when the observed system dynamics fall outside the range of scenarios specified in the OMs over which the MP was demonstrated to be robust [@butterworth2008].
